...
void solve(){
    int n;
    read(n);
    // DP[i] = min cost to go from platform 0 to platform i
    vti a(n), dp(n), p{1};
    read(a);
    dp[0]=0;
    F(i, 1, n){
        // Don't jump last platform
        dp[i]=dp[i-1]+abs(a[i]-a[i-1]);
        if (i>1)
            if (!umin(dp[i], dp[i-2]+3*abs(a[i]-a[i-2])))
                // Shouldn't jump last platform
                p.pb(i);
    }
    //if (n>2 && dp[n-2]+abs(a[n-1]-a[n-2]) < dp[n-3]+3*abs(a[n-1]-a[n-3]))
        //p.pb(n-1);
    p.pb(n);
    print(dp[n-1]);
    print(sz(p));
