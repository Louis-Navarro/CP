...
int dp(vti dis, vt<vti> bls, int i, vt<vti> con){
    if (i==k){
        // TODO: Check conditions
        int ans=0;
        EACH(c, con){
            ans += (int)((dis[c[0]-1] > 0) && (dis[c[1]-1] > 0));
        }
        return ans;
    }

    dis[bls[i][0]] += 1;
    int ans = dp(dis, bls, i+1, con);

    dis[bls[i][0]] -= 1;
    dis[bls[i][1]] += 1;
    umax(ans, dp(dis, bls, i+1, con));
