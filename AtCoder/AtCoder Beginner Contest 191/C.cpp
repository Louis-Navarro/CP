...
int dfs(vt<string> grid, vt<vt<bool>> vis, pii cord, int depth){
    int x = cord.f;
    int y = cord.s;
    int ans = INT;
    ...
    for (int i : {-1, 1, 0})
        for (int j : {-1, 1, 0})
            if (abs(j) != abs(i)){
                if (!vis[x+i][y+j])
                    umin(ans, dfs(grid, vis, MP(x+i, y+j), depth+1));
                else if (x+i == y+j && x+i == 1){
                    return depth + 1
                }
            }
...
void solve(){
    ...
    int ans = dfs(grid, vt<vt<bool>>(h, vt<bool>(w, false)), MP(1, 1), 0)
}
