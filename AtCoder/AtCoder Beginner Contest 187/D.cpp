...
void solve(){
    ...
    sort(rall(h));
    vt<ll> pt(n), pa(n), pb(n);
    pt[0] = h[0].fi, pa[0] = h[0].se.fi, pb[0]=h[0].se.se;
    F(i, 1, n){
        pt[i] = pt[i-1] + h[i].fi;
        pa[i] = pa[i-1] + h[i].se.fi;
        pb[i] = pb[i-1] + h[i].se.se;
    }

    dbg(h[0].fi, h[0].se.fi, h[0].se.se);
    int ans=0, i=-1;
    ll ft=0;
    while (i==-1 || ft <= pa[n-1]-pa[i]){
        ++i, ++ans;
        ft += h[i].se.fi + h[i].se.se;
    }
    print(ans);
}
