...
void solve(){
    ...
    EACH(b, bs){
        for (int i=b.fi+1; i<=h; ++i){
            if (s.find({i, b.se}) != s.end() || bs.find({i, b.se}) != bs.end())
                break;
            ill.insert({i, b.se});
        }
        for (int i=b.fi-1; i; --i){
            if (s.find({i, b.se}) != s.end() || bs.find({i, b.se}) != bs.end())
                break;
            ill.insert({i, b.se});
        }

        for (int j=b.se+1; j<=w; ++j){
            if (s.find({b.fi, j}) != s.end() || bs.find({b.fi, j}) != bs.end())
                break;
            ill.insert({b.fi, j});
        }
        for (int j=b.se-1; j; --j){
            if (s.find({b.fi, j}) != s.end() || bs.find({b.fi, j}) != bs.end())
                break;
            ill.insert({b.fi, j});
        }
