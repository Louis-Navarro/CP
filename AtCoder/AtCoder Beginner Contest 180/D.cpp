...
void solve(){
    ...
    // Max number where mul. is better than add.
    // ka = k + b
    // ka - k = b
    // k(a-1) = b
    // k = b/(a-1)
    // Before k it is best to mult. after it is best to add
    ll k = min(b/(a-1)*a, y-1);

    // How to get from x to k by mult.
    // x * a^z = k
    // a^z = k/x
    // log(a^z) = log(k/x)
    // zlog(a) = log(k/x)
    ...
    // Max adds. left
    // y-1-k == how many x more he can have
    // (y-1-k) / b == how many batches of b he can have
    ans += (y-1-k)/b;
