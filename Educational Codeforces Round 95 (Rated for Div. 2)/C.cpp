...
void solve(){
    ...
    vti a(n);
    read(a);
    vt<vt<vti>> dp(n, vt<vti>(2, vti(2))); // dp[i][j][k] ==> i = boss no. && j = is friend playing? && k = no. played consecutively
    ...
    F(i, 1, n){
        ...
        dp[i][1][0]=min(dp[i-1][0][0], dp[i-1][0][1])+a[i];
        dp[i][1][1]=dp[i-1][1][0]+a[i];
    }
    print(min(min(dp[n-1][0][0], dp[n-1][0][1]), min(dp[n-1][1][0], dp[n-1][1][1])));
}
